...
const server = createServer((request, response) => {
  ...
  request.on('end', () => {
    let name;
    try {
      const pkg = JSON.parse(Buffer.concat(chunks).toString('utf8')).package;
      name = pkg.name;
      // The API contract requires the query to name the Debian ecosystem; reject
      // a request that omits or mis-states it, matching the real advisory service.
